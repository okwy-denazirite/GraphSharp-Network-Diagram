            ...
            where Graph : IBidirectionalGraph<Vertex, Edge>
        {
            ...
            foreach ( Vertex source in g.Vertices )
            {
                ...
                foreach ( Vertex v in undirected.Vertices )
                {
                    ...
                    distances[i, j] = Math.Min( distances[i, j], d ) +  100;
                    distances[i, j] = Math.Min( distances[i, j], distances[j, i] ) + 100;
                    distances[j, i] = Math.Min( distances[i, j], distances[j, i] ) + 100;
                    j++;
                }
                i++;
            }
