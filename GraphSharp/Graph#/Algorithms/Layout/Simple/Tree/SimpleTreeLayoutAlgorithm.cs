        ...
        protected void AssignPositions()
        {
            ...
            foreach ( var layer in layers )
            {
                foreach ( var v in layer.Vertices )
                {
                    ...
                    VertexPositions[v] =
                        changeCoordinates
                            ? new Point( direction * ( (layerSize + size.Height / 2.0) * OkwyScaleFactor), d.position )
                            : new Point( d.position, direction * ( (layerSize + size.Height / 2.0) * OkwyScaleFactor) );
                }
